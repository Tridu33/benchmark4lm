#!/usr/bin/env python3
"""Speech/Audio LLM PEFT Training Profiler (LoRA/QLoRA for Whisper, SpeechT5, etc.)"""
...
def main():
    ...
    csv_logger = CsvTimeSeries(output_dir=args.out_dir, filename="soundlm_peft_timeseries.csv")
    ...
    try:
        if args.task_script:
            ...
            if PEFT_AVAILABLE:
                ...
                print(f"[SoundLM-PEFT] Trainable: {trainable:,}/{total:,} ({100*trainable/max(total,1):.2f}%)")

            model.train()
            optimizer = torch.optim.AdamW([p for p in model.parameters() if p.requires_grad], lr=args.lr)
            dataset = SyntheticAudioDataset(n=args.max_steps*args.batch_size*4)

            print(f"[SoundLM-PEFT] Training {args.max_steps} steps ...")
            t0 = time.time()
            step_times = []
            with TorchProfilerCtx(output_dir=args.out_dir, name="soundlm_peft_profile") as pt_prof:
    ...
    generate_report(result, output_path=os.path.join(args.out_dir, "result.md"),
                    scenario_name=f"Speech PEFT — {args.peft_method}", model_type="soundlm", task_type="training")
